...
int main(int argc, const char* argv[])
{
  ...
  cv::Mat R_rect_00(4, 4, cv::DataType<double>::type);  // 3x3 rectifying
                                                        // rotation to make image
                                                        // planes co-planar
  ...
  std::vector<string> kpDetectors{ "SHITOMASI", "FAST", "BRISK", "ORB", "AKAZE", "SIFT" };
  std::vector<string> kpDescriptors{ "BRIEF", "ORB", "FREAK", "SIFT" };
  std::ofstream outputFile;

  outputFile.open("results.csv");
  outputFile << "TTC per frame/Detector+Desc";
  outputFile << std::endl;
  outputFile << "X,";
  for (int i = 0; i < imgEndIndex; ++i)
  {
    outputFile << "TTC Lidar"
               << ", TTC Camera"
               << ",";
  }
  outputFile << std::endl;
  ...
  for (auto detectorType : kpDetectors)
  {
    for (auto descriptorType : kpDescriptors)
    {
      ...
      outputFile << detectorType << "+" << descriptorType;
      for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
      {
        ...
        float shrinkFactor = 0.10;  // shrinks each bounding box by the given
        // percentage to avoid 3D object merging at the
        // edges of an ROI
        ...
        if (bVis)
        {
          // std::cout << "Showing 3D Objects" << std::endl;
        ...
        if (dataBuffer.size() > 1)  // wait until at least two images have been processed
        {
          ...
          matchBoundingBoxes(matches, bbBestMatches, *(dataBuffer.end() - 2),
                             *(dataBuffer.end() - 1));  // associate bounding boxes
                                                        // between current and
